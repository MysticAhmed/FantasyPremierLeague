...
def performance_averages(goalie_df, defender_df, midfielder_df, forward_df, match_df):
    ...
    midfielder_df.loc[:,'threat_last_5'] = midfielder_df.groupby('player_id')['threat'].rolling(window=6, min_periods=1).mean().reset_index(0, drop=True)
    ...
    forward_df.loc[:,'threat_last_5'] = forward_df.groupby('player_id')['threat'].rolling(window=6, min_periods=1).mean().reset_index(0, drop=True)
