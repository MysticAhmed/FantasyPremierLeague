...
def own_team_predictons(goalie_future_fixture, defender_future_fixture, midfielder_fixtures_df, forward_fixtures_df, data, positions, team_names):
    ...
    # Initialize filtered position lists
    all_goalies = []
    all_defenders = []
    all_midfielders = []
    all_forwards = []

    # Populate lists, ensuring players exist in prediction data
    for player in data['elements']:
        player_id = player['id']
        if positions[player['element_type']] == "Goalkeeper" and player_id in goalie_ids_in_data:
            all_goalies.append(player['web_name'])
        elif positions[player['element_type']] == "Defender" and player_id in defender_ids_in_data:
            all_defenders.append(player['web_name'])
        elif positions[player['element_type']] == "Midfielder" and player_id in midfielder_ids_in_data:
            all_midfielders.append(player['web_name'])
        elif positions[player['element_type']] == "Forward" and player_id in forward_ids_in_data:
            all_forwards.append(player['web_name'])
    ...
    def display_player_in_selected_players(column, player):
        ...
        with column:
            st.markdown(f"<p style='text-align: center; color: yellow; font-size: 17px; margin: 1px 0;'>{player_position}: {player_name}</p>", unsafe_allow_html=True)
            st.markdown(f"<p style='text-align: center; font-size: 17px; margin: 1px 0; color: white;'>Team: {team_name}</p>", unsafe_allow_html=True)
            st.markdown(
            ...
            st.markdown(f"<p style='text-align: center; font-size: 17px; margin: 1px 0; color: lime;'>💲: {player_value / 10} M</p>", unsafe_allow_html=True)
...
def dream_team(upcoming_gameweek, team_names, goalie_future_fixture, defender_future_fixture, midfielder_fixtures_df, forward_fixtures_df):
    ...
    def filter_players_with_combined_limit(players, max_needed, team_counts, position_counts, position):
        ...
        for _, player in sorted_players.iterrows():
            ...
            if team_counts.get(team_name, 0) < MAX_PLAYERS_PER_TEAM and position_counts[position] < max_needed:
    ...
    # Function to display player details in a column, excluding points for bench players
    def display_player_in_column(column, player):
        nonlocal predicted_points, total_price  # Use nonlocal instead of global
        player_id = player["player_id"]
        team_code = player_id_to_team_code.get(player_id)
        player_name = get_player_name(player_id, player_id_to_name)
        player_position = get_player_position(player_id, player_id_to_position)
        player_value = get_player_value(player_id, player_id_to_value)
        team_name = team_names.get(team_code)
